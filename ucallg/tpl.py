...
class Tpl:
    ...
    def uprobe(self, libs: dict[str, dict[int, str]], conf: str) -> str:
        from re import compile, MULTILINE

        comment = compile(r'^//.*$\n?', MULTILINE)
        tpl = self.env.get_template('uprobe.tpl')
        src = tpl.render(libs=Name.map(libs), conf=conf)
        src = comment.sub('', src)
        return src

    def conf(self, libs: dict[str, dict[int, str]]) -> str:
        tpl = self.env.get_template('conf.tpl')
        return tpl.render(libs=Name.map(libs))
